...
public class SurveyTest extends FestSwingJUnitTestCase {
    ...
    public void testRadioButtons() {
        ...
        assertTrue("Option \"siksi\" should be chosen when user clicks it. Check that user interface is big enough so that button is visible!",
                r1.isSelected());
        assertFalse("Option \"se on kivaa\" shouldn't be chosen when option \"siksi\" is clicked",
                r2.isSelected());

        frame.robot.focus(r2);
        frame.robot.click(r2);
        frame.robot.waitForIdle();
        assertTrue("Option \"se on kivaa\" should be chosen when user clicks it.",
                r2.isSelected());
        assertFalse("Option \"siksi\" should be unchosen when option \"se on kivaa\" is clicked.",
                r1.isSelected());

        frame.robot.focus(r1);
        frame.robot.click(r1);
        frame.robot.waitForIdle();
        assertTrue("Option \"siksi\" should be chosen when user clicks it.",
                r1.isSelected());
        assertFalse("Option \"se on kivaa\" should be unchosen when option \"siksi\" is clicked.",
                r2.isSelected());
